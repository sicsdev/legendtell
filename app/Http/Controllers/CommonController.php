...
class CommonController extends Controller
{
    ...
    public function uplodeimages($removeId, $imageid, $pathimport, $before_image_arr)
    {
       ...
       public function uplodeimages_custom($imageid)
       {
           $insert = array();  
           $old_img = array();            
           foreach ($imageid as $key => $file) {
               if(!empty($file)){
                   if (!is_object($file)) {
                       ...
                       $fileName = time() . $key . '.' . $file->getClientOriginalExtension();
                       $file->move(public_path('oil-service'), $fileName);
                       $old_img[$key] = "/oil-service/" . $fileName;
                   } 
               }
                                  
           }
   
           return $ownerdoc = implode(" , ", $old_img);
       }
   
       function uploadimg_custom($imageid)
       {
           $ownerdoc = '';
           ...
           if(!empty($imageid)){
               if (!is_object($imageid)) {
                   ...
                   $fileName = time(). '.' . $imageid->getClientOriginalExtension();
                   $imageid->move(public_path('oil-service'), $fileName);
                   $ownerdoc = "/oil-service/" . $fileName;  
               } 
           }       
       ...
       function uploadpdf_custom($document)
       {
           $pdf_url = '';  
           ...
           if(!empty($document)){
               if (!is_object($document)) {
                   ...
                   $pdf = time() . '.' . $document->getClientOriginalExtension();
                   $document->move(public_path('pdf'), $pdf);
                   $pdf_url = "/pdf/" . $pdf;  
               } 
           }               
