...
// Load Crypto Js Script
(glob.sync('resources/js/crypto/*.js') || []).forEach(file => {
    mix.js(file, `public/${file.replace('resources/', '')}`);
});

(glob.sync('resources/js/crypto/modal/*.js') || []).forEach(file => {
    mix.js(file, `public/${file.replace('resources/', '')}`);
});

(glob.sync('resources/js/crypto/ag-grid/*.js') || []).forEach(file => {
    mix.js(file, `public/${file.replace('resources/', '')}`);
});
...
// amcharts4
mix.copyDirectory('resources/metronic/plugins/amcharts4', 'public/plugins/custom/amcharts4');
