$(document).ready(function () {
    ...
    $(document).on("click", "#savePart", function () {
        ...
        //custom js start........................
        $(document).on("click", ".insertOilServices", function (e) {
            e.preventDefault();
            error = false;
            $(this)
                .closest("form")
                .find("input:not(.editImage,.not_required, :hidden )")
                .each(function () {
                    $(this).removeClass('erradd');
                    $(this).parents(".form-btnw-wrap").removeClass('erradd');
                    ...
                    if ($(this).val() == "") {
                        if ($(this).attr("name") == undefined) {
                            ...
                            $(this).addClass("erradd");
                            $(this).parents(".form-btnw-wrap").addClass('erradd');
    
                            error = true;
                        }
                        if ($(this).attr("type") == "file") {
                            // $(this).siblings('.imgshow').removeAttr('src');
                            // $(this).siblings('.overlay_image').css("display", "none");
                            $(this).parents('.form-btnw-wrap').addClass("erradd");
        ...
        $(".image_uploaded_custom").change(function (event) {
            $(this).parents('.form-btnw-wrap').removeClass("erradd");
            $(this).parents('.pdferrorwraper').find(".red").remove();
    
            var href = URL.createObjectURL(event.target.files[0]);
            var pro_id = this.id;
            var input_file = document.getElementById(pro_id);
            
            var len = input_file.files.length;
            // alert(len);
            $(".display_image_list ul li").remove();
            ...
            // for (var j = 0; j < len; j++) {
                ...
                if (ext != "xls" && ext != "pdf" && ext != "txt" && ext != "csv") {
                    error = true;
                // alert('invalid file');
                $(this).parents('.form-btnw-wrap').addClass("erradd");
                $(".pdferror").after("<p class=red>*Invalid file format please choose only xls,txt,csv,pdf format</p>")
                    return false;
                ...
                $(".display_image_list ul").append(
                    " <li id='" +
                        ...
                        "'><span><button type='button' class='btn cross'  id='" +
                        image_dynamic_id +
                        "'>&nbsp;</button><a href='"+href+ "' target='_blank'><img id='" +
                        image_dynamic_id +
                        "' src='" +
                        src +
                        "' title='" +
                        name +
                        "' class='imgupdate imgshow'></a></span></li>"
                );
        ...
        $('document').ready(function() {  
            $('#SaveOilData').find(".imgshow")
                .each(function () {
                    var src = $(this).attr('src');
                    if(src!= ".."){
                        // $(this).parents('.form-btnw-wrap').find('.overlay_image').css("display", "block");
                        // Commented for hide overlay +  sign on oil serives page
                        $(this).parents('.form-btnw-wrap').find('input[type=file]').addClass("editImage");
                    }          
                });
        });
    
    
        $("body").on("change", ".btn-check_custom", function (e) {
            var Imagename = $(".btn-check_custom").val();
            var Imagename = $(this).val();
            var error = false;
            $(this).parents('.form-btnw-wrap').removeClass("erradd");
            $(this).parents('.form-btnw-wrap').find("label").text('');
    
    
            if (Imagename == "" || Imagename == undefined) {
                error = true;
                $(this).parents('.form-btnw-wrap').addClass("erradd");
                $(this).parents('.form-btnw-wrap').find("label").text('Please Choose Image');
                $(this).siblings('.imgshow').attr('src', '#');
                $(this).siblings('.overlay_image').css("display", "none");
    
    
    
                // $(this).after("<p class=red>*Image Required</p>");
            } else {
                var Image_name = Imagename.split(".");
                var ext = Image_name[Image_name.length - 1];
                if (ext != "png" && ext != "jpeg" && ext != "jpg" && ext != "gif" && ext != "jfif" && ext != "heic") {
                    error = true;
                    $(this).parents('.form-btnw-wrap').addClass("erradd");
                    $(this).parents('.form-btnw-wrap').find("label").text('Invalid Image Format');
                    $(this).siblings('.imgshow').attr('src', '#');
                    $(this).siblings('.overlay_image').css("display", "none");
            ...
            if (error == true) {
                ...
                $(this).siblings('.imgshow').attr('src', URL.createObjectURL(event.target.files[0]));
                $(this).siblings('input[type=hidden]').remove();
                $(this).siblings('.overlay_image').css("display", "block");
            }
        });
